...
scaler_file = "src/Train-Models/scaler_ml.pkl"
with open(scaler_file, "wb") as file:
    pickle.dump(scaler, file)

acc_results = []
for x in tqdm(range(300)):
    x_train, x_test, y_train, y_test = train_test_split(data_scaled, margin, test_size=.1)
    ...
    if acc == max(acc_results):
        model.save_model('Models/XGBoost_{}%_ML-scaled.json'.format(acc))
